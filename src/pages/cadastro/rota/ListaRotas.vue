<template>
  <q-layout-drawer v-model="mostraMenuRight" side="right">
    <q-list>
      <q-list-header>Rotas</q-list-header>
        ...
        <q-infinite-scroll :handler="loadMore" ref="infiniteScroll" >
          ...
          <q-item exact separator link highlight multiline v-for="(rota, index) in listaDeRegistros"  item :to="{ name: 'alterarRota', params: { id: rota.rota} }" :key="index">
            <q-item-main>
              <q-item-tile label> {{ rota.descricao }}</q-item-tile>
              <q-item-tile sublabel>{{ rota.codigo }} </q-item-tile>
            </q-item-main>
          </q-item>
...
export default {
  ...
  methods: {
    loadMore (index, done) {
      console.log('vou atualizar a lista em rotas')
      let inicio = this.inicio
      let fim = this.fim
      let busca = this.busca
      rotaService.procura(busca, inicio, fim)
        .then(result => {
  ...
  mounted () {
    ...
    this.$root.$on('removeRegistro', (rota) => {
      let idRegistro = this.registros.filter(registro => registro.rota === rota)
      this.registros.splice(this.registros.indexOf(idRegistro[0]), 1)
      this.listaDeRegistros = Object.assign({}, this.listaDeRegistros, this.registros)
    })

    this.$root.$on('alteraUnicoRegistro', (novoRegistro) => {
      let idRegistro = this.registros.filter(registro => registro.rota === novoRegistro.rota)
      let id = this.registros.indexOf(idRegistro[0])
      this.registros[id] = new Rota(novoRegistro)
      this.listaDeRegistros = Object.assign({}, this.listaDeRegistros, this.registros)
    })

    this.$root.$on('adicionaRegistroNaLista', (obj) => {
      let rota = new Rota(obj)
      this.registros.push(rota)
      this.listaDeRegistros = Object.assign({}, this.listaDeRegistros, this.registros)
    })
  },
  beforeDestroy () {
